...
func (co *CountWindowIncAggOp) emit(ctx api.StreamContext, errCh chan<- error) {
	...
	results.WindowRange = xsql.NewWindowRange(time.Now().UnixMilli(), time.Now().UnixMilli())
	co.currWindowSize = 0
	co.currWindow = nil
...
func NewTumblingWindowIncAggOp(o *WindowIncAggOperator) *TumblingWindowIncAggOp {
	...
	op.ticker = time.NewTicker(op.Interval)
	return op
}
...
func (so *SlidingWindowIncAggOp) exec(ctx api.StreamContext, errCh chan<- error) {
	...
	for {
		...
		case input := <-so.input:
			...
			switch row := data.(type) {
			case *xsql.Tuple:
				so.gcIncAggWindow(now)
				if so.Delay > 0 {
					so.appendDelayIncAggWindow(ctx, errCh, fv, row)
				}
...
func (so *SlidingWindowIncAggOp) gcIncAggWindow(now time.Time) {
	index := 0
	for i, incAggWindow := range so.currWindowList {
		if now.Sub(incAggWindow.StartTime) >= so.Length {
			index = i + 1
			continue
		}
		break
	}
	if index >= len(so.currWindowList) {
		so.currWindowList = make([]*IncAggWindow, 0)
		return
	}
	so.currWindowList = so.currWindowList[index:]
...
func (so *SlidingWindowIncAggOp) appendDelayIncAggWindow(ctx api.StreamContext, errCh chan<- error, fv *xsql.FunctionValuer, row *xsql.Tuple) {
	name := calDimension(fv, so.Dimensions, row)
	isMatched := so.isMatchCondition(ctx, fv, row)
	if isMatched {
		newDelayWindow := newIncAggWindow(ctx, time.Time{})
